...
| `diagnostic.html` | **Infrastructure Node Diagnostic** — an iOS-styled, tech-tool-feel *simulation* (persistently badged as such) with a **competitor selector**: Spectrum (blue shared trunk, evening bottleneck) or T-Mobile 5G home internet (magenta tower whose signal stalls at the walls; deprioritization, ping, weather callouts), each with a $-waste calculator — and a Brightspeed-yellow **override switch** that transitions the whole UI to the dedicated FTTH state with a brightspeed.com check-availability link |
